  ...
  return (
    <header className="sticky top-0 z-50 bg-blanco-calido/90 backdrop-blur border-b-2 border-crema-2">
      <div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-3">
        ...
        {/* Navegación de escritorio: compacta */}
        <nav
          aria-label="Navegación principal"
          className="hidden md:flex items-center gap-1"
        >
          {/* Explorar: agrupa el catálogo para no saturar la barra */}
          <details className="group relative">
            <summary className="flex cursor-pointer list-none items-center gap-1 whitespace-nowrap rounded-full px-4 py-2 text-sm font-bold text-tinta-suave hover:bg-crema-2 hover:text-tinta transition-colors">
              Explorar
              <span aria-hidden className="text-xs transition-transform group-open:rotate-180">▾</span>
            </summary>
            <div className="absolute left-0 z-50 mt-2 w-52 rounded-2xl border-2 border-crema-2 bg-blanco-calido p-2 shadow-lg">
              {links.map((l) => (
                <LinkNav
                  key={l.href}
                  href={l.href}
        ...
        {/* Menú del avatar (desktop): todo lo personal cuelga de acá */}
        {conSesion && (
          <details className="group relative hidden md:block">
            ...
            >
              <span className="relative flex h-9 w-9 items-center justify-center rounded-full bg-salvia text-sm font-bold text-blanco-calido ring-2 ring-transparent transition group-hover:ring-crema-2">
                {inicial}
                {pendientes > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-terracota-oscuro px-1 text-[10px] font-bold text-blanco-calido">
            ...
            <div className="absolute right-0 z-50 mt-2 w-72 rounded-2xl border-2 border-crema-2 bg-blanco-calido p-2 shadow-lg">
              ...
              <details className="group/notif mt-1 border-t-2 border-crema-2 pt-1">
